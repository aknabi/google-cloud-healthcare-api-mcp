...
export class FhirClient { 
  ...
  constructor(baseUrl: string, useAuth: boolean = false) {
  ...
  setAccessToken(token: string, tokenType: string = 'Bearer') {
    ...
    if (this.accessToken) {
      this.client.defaults.headers.Authorization = `${tokenType} ${this.accessToken}`;
      this.useAuth = true;
    }
  }
  ...
  handleError(error: any) {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.data?.resourceType === 'OperationOutcome' && error.response.data.issue?.length > 0) {
        ...
        errorMessage = `FHIR API error: ${error.response.data.issue.map((i: any) => 
          `[${i.severity}] ${i.diagnostics || i.details?.text || i.code}`
        ).join('; ')}`;
      } else {
